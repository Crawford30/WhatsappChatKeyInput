...
export const ChatInput: React.FC<Props> = ({
  ...
  const { isPanelOpen, panelAnim, togglePanel, closePanel, onInputFocus } =
    useKeyboardManager();
  ...
  const handleEmojiSelect = useCallback(
    (emoji: string) => setText(prev => prev + emoji),
    []
  );

  const handleBackspace = useCallback(() => {
    setText(prev => (prev ? Array.from(prev).slice(0, -1).join('') : prev));
  }, []);
  ...
  // ── Sticker: render as image, no text label ───────────────────────────────
  ...
  // ── GIF: render as image with GIF badge ───────────────────────────────────
...
const styles = StyleSheet.create({
  ...
  panel: { overflow: 'hidden', backgroundColor: '#1F2C34' },
});
